...
(function() {
  ...
  var SUPABASE_ANON_KEY = 'SUPABASE_JWT_REMOVIDO';
  ...
  // Trava de contato (bug Zeom): exige empresa real (nao 'Sem nome'),
  // nome_contato e telefone. Espelha leadIsComplete() da chrome-extension
  // (chrome-extension/sidepanel.js). Sem isso, leads chegam incompletos
  // no Kommo e quebram a sync — motivo pelo qual este arquivo foi removido
  // em cbfd686 e agora restaurado com a trava.
  function validatePayload(payload) {
    var errors = [];
    if (!payload) return { ok: false, errors: ['payload vazio'] };
    if (!payload.empresa || payload.empresa === 'Sem nome') errors.push('empresa obrigatória');
    if (!payload.nome_contato) errors.push('nome_contato obrigatório');
    if (!payload.telefone) errors.push('telefone obrigatório');
...
\
  ...
  $('sh-btn-import').onclick = function() {
    ...
    function importNext() {
      ...
      // empresa nunca cai em 'Sem nome' — validatePayload ja' rejeita esse caso acima.
      payload.empresa = n.payload.empresa;
